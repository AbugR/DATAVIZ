...
stack_bar_graph = go.Figure(data=[
    go.Bar(name='Income', x=data_income.index, y=data_income.values, base=0, marker=dict(color='#bdd392')),
    go.Bar(name='Rent', x=data_rent.index, y=data_rent.values, base=0,
           text=rent_labels, textposition='inside', marker=dict(color='#74913b'))
])
...
def update_cloro_map(year):
    ...
    fig_cloro_map = px.choropleth_mapbox(cloro_df_rent,
                                         ...
                                         zoom=10.5, center={"lat": 41.39, "lon": 2.18},
                                         opacity=0.5,
